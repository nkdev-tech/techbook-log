...
describe('getBooks', () => {
  it('can get books', async () => {
    ...
    const userId = '1'
    const result = await getBooks(userId, [], undefined)

    expect(result).toEqual(mockBooks)
    expect(BookRepository.findAll).toHaveBeenCalledWith(userId, [], undefined)
  })
  ...
  it('can get books filtered by tags', async () => {
    ...
    const userId = '1'
    const result = await getBooks(userId, ['React'], undefined)
  ...
  it('can get books filtered by status', async () => {
    ...
    const userId = '1'
    const result = await getBooks(userId, [], 'unread')

    expect(result).toEqual(mockBooks)
    expect(BookRepository.findAll).toHaveBeenCalledWith(userId, [], 'unread')
  })
})

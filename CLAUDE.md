...
7. レビュー依頼 — 別スレッドでPRを見ながら行う。`web/` のコンポーネントを含む場合は `tailwind-consistency-review` スキルを必ず使ってTailwindの整合性チェックも行う。`legal-compliance-review` スキルを必ず使って利用規約・プライバシーポリシーへの適合チェックも行う
